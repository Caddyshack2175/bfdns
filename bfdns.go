...
* Print any findings
* Have the posssiblity to lookup the geo-loaction of IP addresses (IPv4 or v6)
* Intercation with Shodan online API
ToDo:
- Save to or write to local database or online option
- Save output to file
- possibly use a format, such as JSON
- Writeout to an easy to "report" format (possible csv or tsv)
...
func main() {
	...
	shodanPtr := flag.Bool("shodan", false, "Run top level domain infomation search against Shodan online API.\n\t\t   Please note a Shodan API key will be required for this option.")
	// Parse all the flags
	flag.Usage = func() {
		...
		optional := []string{"t", "geo", "shodan", "shokey"}
	...
	if *shodanPtr && (len(shodanKeyPtr) > 0) {
		fmt.Printf("[+] ... Checking Shodan for top level Domain(s)\n\n")
		...
		fmt.Printf("[+] ... Shodan checks complete\n\n")
	}
}
